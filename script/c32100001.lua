...
function c32100001.filter(c,def)
	return c:IsFaceup() and c:GetDefense()<def and c:IsDestructable()
end
function c32100001.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c32100001.filter(chkc,c:GetDefense()) end
	if chk==0 then return Duel.IsExistingTarget(c32100001.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil,c:GetDefense()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c32100001.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil,c:GetDefense())
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,g:GetCount(),0,0)
end
function c32100001.desfilter(c,e,def)
	return c:IsFaceup() and c:IsRelateToEffect(e) and c:GetDefense()<def
end
function c32100001.desop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	local g=Duel.GetChainInfo(0,CHAININFO_TARGET_CARDS)
	local sg=g:Filter(c32100001.desfilter,nil,e,c:GetDefense())
